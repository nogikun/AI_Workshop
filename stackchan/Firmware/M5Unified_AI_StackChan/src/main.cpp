...
// スタックチャンID
String StackchanID = "4"; // 1,2,3,4 のいずれかを想定（増やす場合はMySQLのデータ数を増やす）
// Model名を問い合わせるサーバーのURL
String serverUrl = "{ここにサーバーのURLを記載してください。}/get_text";//末尾： /get_text は必須です。
...
String json_ChatString = "{\"model\": \"gpt-3.5-turbo-0613\",\"messages\": [{\"role\": \"あなたはスタックチャンです。おはなしをしよう。\", \"content\": \"""\"}]}";
String Role_JSON = "";
...
// モデル名を抜き出す
String extractSubstring(String inputString, String startDelimiter, String endDelimiter) {
  int startIndex = inputString.indexOf(startDelimiter);
  if (startIndex != -1) {
    int endIndex = inputString.indexOf(endDelimiter, startIndex + startDelimiter.length());
    if (endIndex != -1) {
      // startDelimiterとendDelimiterの間の部分を抜き出す
      return inputString.substring(startIndex, endIndex + endDelimiter.length());
    }
  }
  // 開始と終了デリミタが見つからない場合は空文字列を返す
  return "";
}
...
//  M5.Speaker.begin();
  ...
  String urlWithParams = serverUrl + "?number=" + StackchanID; // URLを指定

  Serial.begin(115200);
  HTTPClient http;

  http.begin(urlWithParams); // 送信先のURLを指定
  int httpCode = http.GET(); // GETリクエストを送信

  String input_string = http.getString(); // レスポンスのボディを取得
  //Serial.println('HTTP Response code:');
  //Serial.println(httpCode); // ステータスコードを表示
  Serial.println(input_string); // レスポンスを表示

  http.end(); // リクエスト終了
  String startDelimiter = "\":\"";
  String endDelimiter = "\"";
  // 文字列から"ft:"を検出し、その後の部分を抜き出す
  // 文字列から"ft:"を検出し、それとそれに続く部分を抜き出す
  String model_name = extractSubstring(input_string, startDelimiter, endDelimiter);
  model_name = model_name.substring(3, model_name.length() - 1);
  
  String json_ChatString = "{\"model\": \"gpt-3.5-turbo-0613\",\"messages\": [{\"role\": \"system\", \"content\": \"あなたはスタックチャンです。おはなしをしよう。\"}]}";
  json_ChatString.replace("gpt-3.5-turbo-0613", model_name);
